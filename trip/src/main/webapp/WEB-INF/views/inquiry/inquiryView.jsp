...
<div class="container">
		...
		<table class="table table-bordered">
			...
			<tbody >
				...
				<tr>
					...
					<c:if test="${inquiry.originalFileName ne null }">
						<td>
							<img src="${Path }/resources/inquiry/${inquiry.renameFileName}" width="20">
						</td>
					</c:if>
					<c:if test="${inquiry.originalFileName eq null }">
						<td>첨부파일없음</td>
					</c:if>
				</tr>
				<tr>
	...
	<div>
		...
		<c:if test="${loginMember.userId eq 'admin'}">
			<button type="button"class="btn btn-primary" value="답변" onclick="location.assign('${pageContext.request.contextPath }/replyInquiry/${inquiry.inquiryNo}')">답변</button>
		</c:if>
		<c:if test="${loginMember.userId eq inquiry.writer.userId}">
